...
import { InvalidCredentialsError, UnexpedError } from '@/domain/errors'
...
describe('RemoteAuthentication', () => {
  ...
  test('Should call InvalidCredentialsError if HttpPostClient return 401', async () => {
    const { sut, httpPostClientSpy } = makeSut()
  ...
  test('Should throw UnexpectedError if HttpPostClient return 400', async () => {
    ...
    const promise = sut.auth(mockAuthentication())
    await expect(promise).rejects.toThrow(new UnexpedError())
  })
